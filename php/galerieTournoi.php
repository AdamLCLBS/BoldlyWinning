...
<h1 class=" text-center text-white py-5"><?php echo $h1?></h1> 
        ...
        <?php
        $requete = $pdoTournoi->query("SELECT * FROM tournoi ORDER BY id_tournoi DESC");
       
        while ($card = $requete->fetch(PDO::FETCH_ASSOC)) {
        ?>
        
        <div class="card  col-lg-3 col-md-5 col-9 bg-gris text-white mx-2 mb-4 p-0 h-25">
                <a href="voirTournoi.php?id_tournoi=<?php echo $card['id_tournoi'] ?>"><img src="<?php echo $card['Image']; ?>" class="mh-100 mw-100 " height="300" width="100%" alt="..."></a>
                <div class="card-body " >
